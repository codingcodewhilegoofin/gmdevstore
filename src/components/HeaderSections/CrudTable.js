...
export const CrudTable = props => {
    ...
    const onClickEdit = (data) => {
        ...
        const dateMDY = `${formatDate.getFullYear()}-${formatDate.getMonth() + 1}-${formatDate.getDate()}`;
    ...
    const [getMongoData, setGetMongoData] = useState("https://gmdevapi.com/api/Mongo");
    ...
    const [createMongoData, setCreateGetMongoData] = useState("https://gmdevapi.com/api/MongoCreate/");
    ...
    const handleUserCreateNew = (e) => {
        ...
        async function createMongoDataGmdevapi() {
            try {
                ...
                console.log(`Mongo URL will be: ${createMongoData + e.target[0].value + "/" + e.target[1].value + "/" + e.target[2].value + "/" + e.target[3].value}`);
                const response = await fetch((createMongoData + e.target[0].value + "/" + e.target[1].value + "/" + e.target[2].value + "/" + e.target[3].value));
    ...
    const [updateMongoData, setUpdateMongoData] = useState("https://gmdevapi.com/api/MongoUpdate/");
    ...
    const handleUserInput = (e) => {
        ...
        async function updateMongoDataGmdevapi() {
            try {
                ...
                console.log(`Mongo URL will be: ${updateMongoData + editWindowName + "/" + e.target[0].value + "/" + e.target[1].value + "/" + e.target[2].value + "/" + e.target[3].value}`);
                const response = await fetch((updateMongoData + editWindowName + "/" + e.target[0].value + "/" + e.target[1].value + "/" + e.target[2].value + "/" + e.target[3].value));
    ...
    const [deleteMongoData, setDeleteMongoData] = useState("https://gmdevapi.com/api/MongoDelete/");
    ...
    return (
        <>
            {!currentStatus ? <h2>No data</h2> :

                <>

                    <div className='gio-container-2' style={{ flexDirection: 'row', padding: '1px', margin: '0px', border: '2px solid white' }}>
                        {hideTable ? <></> :
                            <button onClick={() => onClickCreate()} style={{ width: '50%', margin: '5px', padding: '5px', backgroundColor: 'white', fontSize: 'xx-large' }}> 🆕 </button>
                        }
                        <button onClick={() => onClickHide()} style={{ width: (!hideTable ? '75%' : '100%'), margin: '5px', padding: '5px', backgroundColor: 'black' }}> {hideTable ? '👁️' : 'Hide Table👁️'} </button>
                    </div>
                        ...
                        <>
                            <div className={editWindow ? "gio-container" : ""} style={{ width: `${mainDivSize}`, margin: '5px', border: 'none', background: 'none' }}>
                                ...
                                {editWindow ?
                                    <>
                                        <form
                                            onSubmit={handleUserCreateNew}
                                        >
                                            <div style={{ width: "100%", height: "100%", backgroundColor: 'black', padding: '10px' }}>
                                                <p style={{ color: 'white' }}>Create new record in database</p>
                                                <p style={{ color: 'white' }}>Name</p>
                                                <input on type="text" style={{ border: 'none', width: `${mainDivSize}`, backgroundColor: 'white', borderRadius: '10px' }} />
                                                <p style={{ color: 'white' }}>Socail Media</p>
                                                <input on type="text" style={{ border: 'none', width: `${mainDivSize}`, backgroundColor: 'white', borderRadius: '10px' }} />
                                                <p style={{ color: 'white' }}>Website / Project</p>
                                                <input on type="text" style={{ border: 'none', width: `${mainDivSize}`, backgroundColor: 'white', borderRadius: '10px' }} />
                                                <p style={{ color: 'white' }}>Date</p>
                                                <input on type="date" style={{ border: 'none', width: `${mainDivSize}`, backgroundColor: 'white', borderRadius: '10px' }} />
                                            ...
                                            <input
                                                ...
                                                value="Enter"
                                            />
                                            <button style={{ border: '1px solid white', color: 'white' }} onClick={cancelCreateNewWindow}>Cancel</button>
                                        </form>
                                    </>
                                    :
                                    <></>
                                        ...
                                        <tr style={{ border: 'none' }}>
                                            {!editWindow2 ?
                                                <>
                                                    ...
                                                    <th style={{ padding: '5px', borderRight: '2px solid black', }}>Website</th>
                                                    <th style={{ padding: '5px', borderRight: '2px solid black', }}>Social Media</th>
                                                    <th style={{ padding: '5px', borderRight: '2px solid black', }}>Date Created</th>
                                                </>
                                                :
                                                <></>
                                            }
                                                ...
                                                <tr style={{ border: 'none' }}>
                                                    ...
                                                    <td style={{ border: '2px solid white' }}>
                                                        <div className='gio-container-2' style={{ flexDirection: `${btnListDirection}`, padding: '0px', margin: '0px', border: '2px solid white' }}>
                                                            ...
                                                            <button onClick={() => onClickEdit(data)} style={{ width: `${editBtnSize}`, margin: '1px', backgroundColor: 'green' }}> ✏️ </button>
                                                            <button onClick={() => onClickDelete(data)} style={{ width: `${editBtnSize}`, margin: '1px', backgroundColor: 'red' }}> 💣 </button>
                                                        </div>
                            ...
                            >
                                <div className={editWindow2 ? "gio-container" : ""} style={{ width: `${mainDivSize}`, margin: '5px', border: 'none', background: 'none' }}>
                                    ...
                                    {editWindow2 ?
                                        <>
                                            <div style={{ width: "100%", height: "100%", backgroundColor: 'black', padding: '10px' }}>
                                                <p style={{ color: 'white' }}> <b style={{ color: 'lightgreen' }}>Edit</b> record for {editWindowName} </p>
                                                <p style={{ color: 'white' }}>Name</p>
                                                <input on type="text" value={editWindowData.name} style={{ border: 'none', width: `${mainDivSize}`, backgroundColor: 'white', borderRadius: '10px' }} />
                                                <p style={{ color: 'white' }}>Social Media</p>
                                                <input on type="text" style={{ border: 'none', width: `${mainDivSize}`, backgroundColor: 'white', borderRadius: '10px' }} />
                                                <p style={{ color: 'white' }}>Website / Project</p>
                                                <input on type="text" style={{ border: 'none', width: `${mainDivSize}`, backgroundColor: 'white', borderRadius: '10px' }} />
                                                <p style={{ color: 'white' }}>Original Creation Date</p>
                                                <input type="date" value={editWindowDate} style={{ border: 'none', width: `${mainDivSize}`, backgroundColor: 'white', borderRadius: '10px' }} />
                                            ...
                                            <input
                                                ...
                                                value="Enter"
                                            />
                                            <button style={{ border: '1px solid white', color: 'white' }} onClick={cancelEditWindow}>Cancel</button>
